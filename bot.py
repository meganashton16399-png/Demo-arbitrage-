...
# Ab Token code me nahi, Render ke Environment variables se aayega
APP_ID = 119348
API_TOKEN = os.environ.get("DERIV_TOKEN") # ✅ Hidden
TELE_TOKEN = os.environ.get("BOT_TOKEN")   # ✅ Hidden
MY_CHAT_ID = os.environ.get("CHAT_ID")     # ✅ Hidden

# Agar galti se Env set nahi kiya to error na aaye, isliye check
if not API_TOKEN or not TELE_TOKEN:
    print("❌ ERROR: Tokens nahi mile! Render Environment Check karo.")
...
def send_proposal(ws, direction, amount):
    ...
    try:
        ...
        bot.send_message(MY_CHAT_ID, f"🔫 Preparing: {direction.upper()} | Stake: ${amount}")
...
def start_bot(message):
    ...
    bot.send_message(message.chat.id, f"🚀 Secure Bot Active: {SELECTED_SYMBOL}", reply_markup=types.ReplyKeyboardRemove())
